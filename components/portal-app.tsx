...
import { getApiUrl, getAuthUrl, loadPortalData, postPortalAction } from '@/lib/api';
...
export function PortalApp() {
  ...
  useEffect(() => { setAuthUrl(getAuthUrl()); if (!getApiUrl()) return; loadPortalData().then((remote) => { if (remote) { setData(remote); setConnected(true); } }).catch((error: Error) => setMessage(error.message)); }, []);
